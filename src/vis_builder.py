...
class VisualisationBuilder:
	...
	def set_rod_content(self, rod, content):
		if(rod != -1):		
			self.rod_content[rod] = copy.copy(content)
	...
	def draw_all(self):
		...
		for i in range(3):
			rod_copy = copy.copy(self.rod_content[i])
			for j in range(len(self.rod_content[i])):
				self.draw_disk(self.rod_content[i].pop(), i)
			self.rod_content[i] = rod_copy
