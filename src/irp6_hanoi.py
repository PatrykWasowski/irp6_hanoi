...
DISK_STOCK = 0.0	# zapas na szerokosc chwytaka przy podejmowaniu krazka odpowiedniego koloru
is_above = 0
move = [0, 0]
starting_rod = -1000
running = 0
...
class Irp6Hanoi:
	...
	def move_to_starting_pose(self):
		...
		self.irpos.tfg_to_joint_position(RED_DISK+0.005, 3.0)
	...
	def move_to_rod(self, end):		
		...
		if(self.rod_position[end] != [0, 0, 0, 0, 0, 0]):
			...
			diff = (end - self.current_rod) * 0.074 
			self.irpos.move_rel_to_cartesian_pose(7.0,  Pose(Point(0.0, diff, 0.0), Quaternion(0.0, 0.0, 0.0, 1.0)))
		...
		while (not is_above):
			if((not is_above) and (move != [0, 0])):
				running = 1				
				self.irpos.move_rel_to_cartesian_pose(3.0, Pose(Point(-1*move[1], move[0], 0.0), Quaternion(0.0, 0.0, 0.0, 1.0)))
	...
	def grab_disk(self, disk, disks_below):
		...
		self.irpos.move_rel_to_cartesian_pose_with_contact(5.0, Pose(Point(0.0, 0.0, distance_to_disk), Quaternion(0.0, 0.0, 0.0, 1.0)), Wrench(Vector3(0.0,0.0,8.0),Vector3(0.0,0.0,0.0)))
		#self.irpos.tfg_to_joint_position(self.get_disk_size(disk), 3.0)
		print "Zlapalem dysk"
		#self.irpos.move_rel_to_cartesian_pose(5.0, Pose(Point(0.0, 0.0, (-1.0)*distance_to_disk), Quaternion(0.0, 0.0, 0.0, 1.0)))
					
	def leave_disk(self):
		print "Zrzucam dysk"
		self.irpos.move_rel_to_cartesian_pose_with_contact(5.0, Pose(Point(0.0, 0.0, 2.0), Quaternion(0.0, 0.0, 0.0, 1.0)))
	...
	def shift_disk(self, src, dst, disk, disks_below):
		...
		rospy.sleep(1.)		# to be sure that new rod is found
		
		move_to_rod(src) 	
		
		self.pub.publish(dst)			# before robot moves to destination rod, the rod is already found 
