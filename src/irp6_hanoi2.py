...
class Irp6Hanoi:
	...
	def add_rviz_rod(self, rod):
		...
		marker.pose.position.z = actual.position.z - 0.108 + rod * 0.0045
		...
		if(rod == starting_rod):
			self.add_rviz_disks(4, actual)
			self.add_rviz_disks(3, actual)
			self.add_rviz_disks(2, actual)
			self.add_rviz_disks(1, actual)


	def add_rviz_disks(self, disk, act):
		marker = Marker()
		...
		marker.pose.position.z = act.position.z - 0.143 + 0.007 + (4 - disk) * 0.01
		marker.pose.orientation.w = 1.0
		marker.scale.z = 0.01
	...
	def move_disk_rviz(self, disk):
		marker = Marker()
		actual = self.irpos.get_cartesian_pose()

		marker.id = disk
		marker.action = marker.ADD
		marker.header.frame_id = "/pl_g1"
		marker.pose.position.x = actual.position.x
		marker.pose.position.y = actual.position.y
		marker.pose.position.z = actual.position.z - 0.04
